...
function connect(){console.log('Connecting...');state.ws=new WebSocket(WS_URL);state.ws.onopen=function(){console.log('Connected');};state.ws.onclose=function(){setTimeout(connect,3000);};state.ws.onerror=function(e){console.error('WS error',e);};state.ws.onmessage=function(e){handleMessage(JSON.parse(e.data));};}
function showView(id){qSA('.main-view').forEach(function(v){v.classList.remove('active');});var el=document.getElementById(id);if(el)el.classList.add('active');}
...
function renderMembers(){var srv=state.servers.get(state.currentServer);var ol=qS('#members-online');var ofl=qS('#members-offline');if(!srv||!ol||!ofl)return;var mems=srv.members||[];var on=[],off=[];for(var i=0;i<mems.length;i++){var id=mems[i];var f=state.friends.get(id);if(id===state.userId||(f&&f.status==='online'))on.push(id);else off.push(id);}var oc=qS('#online-count');var ofc=qS('#offline-count');if(oc)oc.textContent=on.length;if(ofc)ofc.textContent=off.length;var oh='',ofh='';for(var j=0;j<on.length;j++)oh+=memberHTML(on[j]);for(var k=0;k<off.length;k++)ofh+=memberHTML(off[k]);ol.innerHTML=oh;ofl.innerHTML=ofh;}
function memberHTML(id){var isMe=id===state.userId;var f=state.friends.get(id);var n=isMe?state.username:(f?f.name:'User');var av=isMe?state.userAvatar:(f?f.avatar:null);var srv=state.servers.get(state.currentServer);var isOwner=srv&&srv.ownerId===id;var crownSvg='<svg class="crown-icon" viewBox="0 0 24 24" fill="currentColor"><path d="M5 16L3 5l5.5 5L12 4l3.5 6L21 5l-2 11H5zm14 3c0 .6-.4 1-1 1H6c-.6 0-1-.4-1-1v-1h14v1z"/></svg>';return'<div class="member-item" data-id="'+id+'"><div class="avatar online">'+(av?'<img src="'+av+'">':n.charAt(0).toUpperCase())+'</div><span>'+escapeHtml(n)+(isOwner?' '+crownSvg:'')+'</span></div>';}
...
function messageHTML(m){var t=m.time||new Date(m.timestamp||Date.now()).toLocaleTimeString('ru-RU',{hour:'2-digit',minute:'2-digit'});var a=m.author||m.username||'User';var isOwn=m.oderId===state.userId;var txt=m.text||m.content||'';var replyHtml='';if(m.replyTo){var ra=m.replyTo.author||'?';var rav=m.replyTo.avatar;var isDeleted=m.replyTo.deleted;if(isDeleted){replyHtml='<div class="message-reply deleted"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><line x1="15" y1="9" x2="9" y2="15"/><line x1="9" y1="9" x2="15" y2="15"/></svg><span class="reply-content">Сообщение удалено</span></div>';}else{replyHtml='<div class="message-reply" data-reply-id="'+m.replyTo.id+'"><div class="reply-avatar">'+(rav?'<img src="'+rav+'">':ra.charAt(0).toUpperCase())+'</div><span class="reply-author">'+escapeHtml(ra)+'</span><span class="reply-content">'+escapeHtml(m.replyTo.text||'').substring(0,50)+'</span></div>';}}return'<div class="message'+(m.replyTo?' has-reply':'')+'" data-id="'+m.id+'" data-author-id="'+(m.oderId||'')+'" data-author="'+escapeHtml(a)+'" data-text="'+escapeHtml(txt)+'">'+replyHtml+'<div class="message-body"><div class="avatar">'+(m.avatar?'<img src="'+m.avatar+'">':a.charAt(0).toUpperCase())+'</div><div class="content"><div class="header"><span class="author">'+escapeHtml(a)+'</span><span class="time">'+t+'</span></div><div class="text">'+escapeHtml(txt)+'</div></div></div></div>';}
...
if(logBtn){logBtn.onclick=function(){console.log('Login clicked');var em=qS('#login-email');var pw=qS('#login-pass');var er=qS('#login-error');var emv=em?em.value.trim():'';var pwv=pw?pw.value:'';if(!emv||!pwv){if(er)er.textContent='Заполните все поля';return;}if(!send({type:'login',email:emv,password:pwv})){if(er)er.textContent='Нет подключения';}};}
